...
JSON-RPC channel 默认不启动，只有显式打开开关后才会被 `bub gateway` 拉起：
...
```bash
export BUB_JSONRPC_ENABLED=true
```

### 监听地址配置

默认监听 `127.0.0.1:8420`，可通过环境变量修改：

```bash
export BUB_JSONRPC_HOST=0.0.0.0   # 监听所有接口
export BUB_JSONRPC_PORT=9000       # 使用 9000 端口
```
...
4. 执行 `uv run philip rpc.chat ws=true stream=true session=demo`
...
- 默认关闭；需显式设置 `BUB_JSONRPC_ENABLE=true`
