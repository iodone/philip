---
name: workflow-analytical-writing
description: 将已验证的调研素材转化为有判断力的 external-facing 分析文章 不适用于：素材未经验证就开写（先走 workflow-deep-research-survey）；内部纪要/沉淀类整理直接写 contexts/，不需要 Thesis Catalog。
---
...
- **前置依赖**: 深度调研工作流（`workflow-deep-research-survey`）的 Phase 1-3 产出
- **创建日期**: 2026-04-28
- **最后更新**: 2026-04-28
...
2. 搜索作者已有的相关写作。在 `contexts/blog/`、`contexts/survey_sessions/` 中查找与当前话题直接相关的已有文章和调研。记录具体的文件路径和相关段落。
...
**控制 cognitive burden**。读者在任何一段里同时接收的新概念不超过两个。如果一段需要引入新术语或新框架，先用一个读者已有的场景或体感把它接住，再给名字。不要连续三段都在引入新概念而不给读者消化的空间。判断标准：把文章读给目标读者画像里的人听，如果他在某一段开始走神或回翻，大概率是那一段的新概念密度太高了。
...
4. **分析框架是 internal 工具，不进入成品。** Thesis Catalog（L1-L6）、Phase 名称、"叙事重构"这类术语，全部属于写作过程的脚手架，不能出现在最终文章里。读者应该感受到的是一个有自己思考方式的人在做分析，不是一个框架在被套用。具体来说：不要写"从技术族谱的角度看"，直接写三代演化的事实；不要写"这里的瓶颈发生了迁移"，直接写代码变便宜之后验证成了最慢的环节。框架指导你怎么想，但读者看到的只有想出来的结果。
...
| 内部框架泄露到成品 | 文中出现"从 L4 技术族谱视角"、"叙事重构"等术语 | Thesis Catalog、Phase 名称全部是写作脚手架，最终文章里零出现 |
