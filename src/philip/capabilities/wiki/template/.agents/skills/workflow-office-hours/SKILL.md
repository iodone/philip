...
description: |
  ...
  Do NOT use for: the build decision is already made and an executable spec is wanted (use spark), or pure technical implementation discussion.
---
...
- **类型**: Workflow
- **适用场景**: 产品诊断、设计思维头脑风暴、创业方向验证
- **输出位置**: `contexts/thought_review/`
- **创建日期**: 2026-05-22
...
Extract 3-5 significant keywords from the user's problem statement and grep across design docs in `contexts/thought_review/`.
...
Write the design document to `contexts/thought_review/`.

**Design lineage:** Before writing, check for existing design docs:
```bash
ls -t contexts/thought_review/*-design-*.md 2>/dev/null
```
If prior designs exist, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain.

Write to `contexts/thought_review/{date}-{slug}-design.md`:
...
- **Implementation planning** — for well-scoped work, proceed to a plan
- **Deep research** — if the design needs more validation data
- **Prototype** — if the user wants to explore the design interactively

The design doc at `contexts/thought_review/` is automatically discoverable by downstream skills.
